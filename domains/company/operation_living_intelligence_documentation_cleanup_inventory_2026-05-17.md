...
status: active_folder_reduced_wave_provenance_archived
---
...
Older Wave 1-9 documents are no longer active operating instructions. They are archived provenance.
...
| Transition summaries | 2 | Deferred return map and Waves 1-9 consolidation summary. |
...
```text
31 Operation Living Intelligence Markdown docs in domains/company
```
...
```text
108 Operation Living Intelligence Markdown docs
```
...
- If a deferred lane becomes relevant again, start from the active reference map and the deferred return map, then open archived wave files only for provenance.
